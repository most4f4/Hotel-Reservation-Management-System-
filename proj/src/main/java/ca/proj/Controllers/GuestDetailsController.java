...
public class GuestDetailsController {
    ...
    @FXML void submitButtonPressed(ActionEvent event) {
        ...
        ReserveConfirmationController controller = (ReserveConfirmationController) HotelReservationApplication.getControllers().get(SceneName.ConfirmationRes);

        controller.setFields(reservation.getReservationID(), guest.getName(), guest.getEmail(), guest.getPhoneNumber(),
                String.valueOf(reservation.getNumberOfGuests()), String.valueOf( reservation.getCheckInDate()), String.valueOf( reservation.getCheckOutDate()), roomsList, totalPayable);

        logger.info("Reservation created: " + reservation.getReservationID());
        getStage().setTitle("Reservation Confirmation");
        getStage().setScene(HotelReservationApplication.getScenes().get(SceneName.ConfirmationRes));
